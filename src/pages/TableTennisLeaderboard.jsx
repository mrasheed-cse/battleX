...
export default function TableTennisLeaderboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null)
    try {
      const url = `${API}?view=leaderboard&period=${period}&opponent=${opponent}&limit=100`
      const res = await fetch(url)
      const json = await res.json()
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`)
      // handle both { entries:[] } and { data:[] } and plain []
      const data = json.entries || json.data || json || []
      setEntries(Array.isArray(data) ? data : [])
      if (Array.isArray(data) && data.length === 0) {
        console.log('[TT LB] raw response:', json)
      }
    } catch (e) {
      console.error('[TT LB] error:', e.message)
