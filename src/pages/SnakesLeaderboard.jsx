...
const POS_MEDAL  = { 1:'🥇', 2:'🥈', 3:'🥉', 4:'4th', 5:'5th', 6:'6th' }
const POS_SHORT  = { 1:'1st', 2:'2nd', 3:'3rd', 4:'4th', 5:'5th', 6:'6th' }
const POS_LABEL  = { 1:'1st Place', 2:'2nd Place', 3:'3rd Place', 4:'4th Place', 5:'5th Place', 6:'6th Place' }
const POS_PTS    = { 1:100, 2:90, 3:80, 4:70, 5:60, 6:50 }
          ...
          <div className={styles.winnerBanner}>
            ...
            <div>
              ...
              <div className={styles.winnerSub}>
                Winner · Score: {winner.score} · {fmtMs(winner.duration_ms)}
              </div>
            </div>
                ...
                <tr key={p.id} className={p.position === 1 ? styles.modalWinRow : ''}>
                  <td>
                    <div className={styles.rankCell}>
                      <span className={styles.rankMedal}>{p.position <= 3 ? POS_MEDAL[p.position] : ''}</span>
                      <span className={styles.rankText}>{POS_SHORT[p.position] || p.position}</span>
                    </div>
                  </td>
                  <td><span className={styles.modalPlayerName}>{p.player_name}</span></td>
                  <td><span className={styles.scoreChip}>{p.score ?? '—'}</span></td>
                  <td><span className={styles.timeCell}>{fmtMs(p.duration_ms)}</span></td>
                  <td>
                    <span className={styles.ptsCell}
                      style={{ color: p.position===1?'#ffd700':p.position===2?'#c0c0c0':p.position===3?'#cd7f32':'#90caf9' }}>
                      +{p.points || POS_PTS[p.position] || 50}
                    </span>
                  </td>
                  <td><span className={styles.matchIdSmall}>{shortId(p.match_id)}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            return (
              <div key={pos} className={`${styles.podiumSlot} ${styles['podiumPos'+pos]}`}>
                ...
                <div className={styles.podiumBlock}>
                  ...
                  <span className={styles.podiumScore}>Score: {p.score}</span>
...
function MatchRow({ match, highlight, onSelect }) {
  const winner = match.players.find(p => p.position === 1)
  const isHL   = highlight && match.players.some(p =>
    p.player_name.toLowerCase().includes(highlight.toLowerCase()))

  return (
    <div className={`${styles.matchRow} ${isHL ? styles.matchRowHL : ''}`}
        ...
        <div className={styles.mrPlayers}>
          {match.players.map(p => (
            <span key={p.id} className={styles.mrPlayerChip}
              style={{ fontWeight: p.position === 1 ? 800 : 500 }}>
              {p.position <= 3
                ? <span className={styles.mrMedal}>{POS_MEDAL[p.position]}</span>
                : <span className={styles.mrPos}>{p.position}</span>
              }
              {p.player_name}
              {p.score > 0 && <span className={styles.mrScore}>{p.score}</span>}
            </span>
          ))}
        </div>
      </div>
      <div className={styles.mrRight}>
        ...
        {match.duration_ms > 0 && <span className={styles.mrDur}>⏱ {fmtMs(match.duration_ms)}</span>}
      ...
      <table className={styles.table}>
        <thead>
          <tr>
            ...
            <th className={styles.tc}>Avg Score</th>
          ...
          {rankings.map((r, i) => {
            ...
            return (
              <tr key={r.player_name}
                ...
                <td className={styles.tc}><span className={styles.bestPos}>{POS_SHORT[r.best_position]||r.best_position}</span></td>
                <td className={styles.tc}><span className={styles.scoreVal}>{r.avg_score}</span></td>
                <td className={styles.tc}><span className={styles.timeVal}>{fmtMs(r.avg_time_ms)}</span></td>
                <td className={styles.tc}><span className={styles.ptsBadge}>{r.total_points}</span></td>
              </tr>
            )
          })}
        </tbody>
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [mRes, rRes] = await Promise.all([
        fetch(`${API}?view=matches&period=${period}&limit=100${q}`),
        fetch(`${API}?view=rankings&period=${period}&limit=100${q}`),
      ])
      ...
      <div className={styles.header}>
        <div className="container">
          ...
          <div className={styles.heroRow}>
            ...
            <div>
              <h1 className={styles.heroTitle}>Snakes &amp; Ladders Leaderboard</h1>
              <p className={styles.heroSub}>Match history, player rankings &amp; detailed stats</p>
            </div>
            <Link to="/games/snakes-and-ladders/play" className={styles.playNowBtn}>▶ Play Now</Link>
          </div>
        </div>
      </div>
      ...
      <div className="container">
        ...
        <div className={styles.statsGrid}>
          {[
            ...
            { icon:'👥', val: rankings.length,  label:'Players'       },
        ...
        {view === 'rankings' && (
          <div className={styles.ptsKey}>
            <span className={styles.ptsKeyLabel}>Points:</span>
            {Object.entries(POS_PTS).map(([pos, pts]) => (
              <span key={pos} className={styles.ptsKeyItem}>
                {POS_MEDAL[pos]||POS_SHORT[pos]} = {pts}pts
              </span>
            ))}
          </div>
        )}
        ...
        {/* Content */}
        {error ? (
          <div className={styles.errorBox}>⚠ {error}
